...
function App() {
  ...
  const { lists, activeList } = useSelector(({ lists }) => lists);
  useEffect(() => {
    dispatch(fetchLists());
  ...
  }, [lists, dispatch, location.pathname]);
  return (
    <div className="todo">
      <div className="todo__sidebar">
        <List
          onActiveItem={(list) => navigate('/')}
          items={[
            {
          ...
          <Route
            ...
            element={
              ...
              lists.map((list, index) => {
                return <Tasks key={list + index} items={list} />;
              })
            }
          />
          <Route path={`/lists/:id`} element={<Tasks items={activeList} />} />
        </Routes>
      </div>
    </div>
  );
