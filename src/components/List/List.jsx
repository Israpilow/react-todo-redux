...
import { setLists, setRemoveList, setActiveList } from '../../redux/actions/lists';

import './List.scss';
import '../Badge/Badge.scss';

const List = ({ items, onClick, activeList, onRemove, isRemoved, onActiveItem }) => {
  const dispatch = useDispatch();

  const onRemoveList = (item) => {
    if (window.confirm('Вы действительно хотите удалить список ?')) {
      dispatch(setRemoveList(item));
      axios.delete('http://localhost:3001/lists/' + item.id).catch(() => {
        alert('Произошла ошибка при удалении списка. Попробуйте еще раз, перезагрузив страницу');
      });
    }
  };
            ...
            <li
              ...
              <i>
                {item.icon ? (
                  ...
                  <i className={`badge badge--${item.color && item.color.name}`}></i>
                )}
              </i>
